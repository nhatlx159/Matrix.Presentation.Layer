...
import { getDataForYou } from '../../../api_gateway/apiRequest';

function YouMightLike(props) {
    
  const [data, setData] = useState(JSON.parse(localStorage.getItem('foryou')));
  useEffect( () => {
    getDataForYou()
    setData(JSON.parse(localStorage.getItem('foryou')))
    console.log(data);
  }, [localStorage.getItem('foryou')])
  useEffect( () => {
    getDataForYou()
    setData(JSON.parse(localStorage.getItem('foryou')))
    console.log(data);
